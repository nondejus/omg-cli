...
const { omgJSMain, childChain, getFee } = require("./index");

let processedUTXOPos: any[] = [];
let utxos: any[] = [];
const runInParallel = 2;
  ...
  const supportedCurrencies = [
    {
      name: "ETH",
      address: ethAddress,
      amount: new BigNumber(await getFee(ethAddress))
    },
    {
      name: "FEE",
      address: feeAddress,
      amount: new BigNumber(await getFee(feeAddress))
    }
  ];
  while (true) {
    utxos = await childChain.getUtxos(config.charlie_eth_address);

    async.eachLimit(utxos, runInParallel, async (utxo: any, done: any) => {
      ...
      if (!processedUTXOPos.includes(utxo.utxo_pos)) {
        for (const supportedCurrency of supportedCurrencies) {
          if (
            ...
            //      console.log(`${JSONbig.stringify(tx, undefined, 2)}`);
            ...
            const txReceipt = await omgJSMain(cliOptionSendTx);
